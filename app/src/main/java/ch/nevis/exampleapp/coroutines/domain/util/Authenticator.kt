 ...
 */
fun Authenticator.isUserEnrolled(username: String, allowClass2Sensors: Boolean): Boolean = when (val userEnrollment = userEnrollment()) {
 ...
 */
fun Authenticator.titleResId(): Int = when (val aaid = aaid()) {
